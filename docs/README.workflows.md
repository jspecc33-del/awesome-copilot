...
| [python-build-check](../workflows/python-build-check.md) | GitHub Actions CI workflow for Python projects: runs pytest with JSON reporting, uploads the report as an artifact, and posts a score/summary comment on every pull request. Supports Postgres and Redis service containers for integration tests. Fails the build on any test errors. | N/A |
